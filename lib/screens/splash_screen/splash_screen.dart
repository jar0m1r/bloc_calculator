...
class SplashScreen extends HookWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final vectorManager = context.watch<VectorManagerBloc>();
    final state = vectorManager.state;
          ...
          child: Center(
            child: state is ManagerLoading
                ? CircularProgressIndicator(
                    value: state.total != 0 ? state.count / state.total : 0.0)
                : AspectRatio(
                    aspectRatio: (state as ManagerReady)
                        .manager
                        .sheetMap['splash']
                        .size
                        .aspectRatio, //!
                    child: LayoutBuilder(
                      builder: (context, constraints) {
                        final group =
                            (state as ManagerReady).manager.sheetMap['splash'];
                        final scaleFactor =
                        ...
                        return Stack(
                          ...
                          children: [
                            ...
                            //equals sign
                            StaggerVectorAnimation(
                              element: group.elementMap['symbol_equals_bottom'],
                              controller: controller,
                            ...
                            StaggerVectorAnimation(
                              element: group.elementMap['symbol_equals_top'],
                              controller: controller,
                              beginOffset: const Offset(400, 0),
                              interval: const Interval(0.6, 1.0,
                                  curve: Curves.easeOut),
...
//DropIn.fromTheLeft(TravelStraightPath, elevation: 3)
//SlideIn.fromTheRight(TravelCurvedPath, )
